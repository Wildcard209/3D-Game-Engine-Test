...
namespace OpenGL_Game.Managers
{
    static class ResourceManager
    {
        ...
        public static Geometry LoadGeometry(string filename, string baseName)
        {
            Geometry geometry;
            geometryDictionary.TryGetValue(filename, out geometry);
            if (geometry == null)
            {
                geometry = new Geometry();
                geometry.LoadObject(filename, baseName);
                geometryDictionary.Add(filename, geometry);
            }
        ...
        public static Geometry LoadGeometry(string filename, string baseName, string normalName)
        {
            Geometry geometry;
            geometryDictionary.TryGetValue(filename, out geometry);
            if (geometry == null)
            {
                geometry = new Geometry();
                geometry.LoadObject(filename, baseName,normalName);
                geometryDictionary.Add(filename, geometry);
            }
        ...
        {
            int address;
            shaderDictionary.TryGetValue(filename, out address);
            if (address == 0)
            {
                ...
                shaderDictionary.Add(filename, address);
            }
        ...
        public static EntityManager LoadEntitysFromFile(string filepath)
        {
            ...
            List<Entity> entities = JsonSerializer.Deserialize<List<Entity>>(json);
        ...
        public static void ExportEntitys(EntityManager entitys)
        {
            StreamWriter writer = new StreamWriter("Entitys.txt");
            string jsonString = JsonSerializer.Serialize(entitys.Entities());

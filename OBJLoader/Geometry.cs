...
namespace OpenGL_Game.OBJLoader
{
    public class Group
    {
        ...
        public int textureNormal = 0;
        public int textureHight = 0;
    ...
    public class Geometry
    {
        ...
        public void LoadObject(string filename, string filenameBase = "", string filenameNormal = "")
        {
            try
            {
                ...
                foreach (var group in obj.Groups)
                {
                    ...
                    else if(filenameBase != "")
                    {
                        newGroup.texture = ResourceManager.LoadTexture(filenameBase);
                    }
                    else
                    {
                    ...
                    if (filenameNormal != "")
                    {
                        newGroup.textureNormal = ResourceManager.LoadTexture(filenameNormal);
                    }
                    bool error = false;
                    string errorMessage = "";
        ...
        public void Render(int uniform_diffuse)  // OBJ CHANGED
        {
            foreach (var group in groups)
            {
                if (group.texture > 0)
                {
                    ...
                    if (group.textureNormal > 0)
                    {
                        GL.ActiveTexture(TextureUnit.Texture1);
                        GL.BindTexture(TextureTarget.Texture2D, group.textureNormal);
                        GL.ActiveTexture(TextureUnit.Texture0);
                    }
